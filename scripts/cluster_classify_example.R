...
N = 30
gene_df = rbind(tibble(gene_a = rnorm(n=N, mean=4.5), gene_b = rnorm(n=N, mean=1), explant_id = seq(1, N), type = 1),
                tibble(gene_a = rnorm(n=N, mean=1), gene_b = rnorm(n=N, mean=1), explant_id = seq(N+1, 2*N), type = 2),
                tibble(gene_a = rnorm(n=N, mean=1), gene_b = rnorm(n=N, mean=4.5), explant_id = seq((2*N)+1, 3*N), type = 3))
...
knn_classification = predict(knn_fit, new_samples, type="class")
